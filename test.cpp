...
int16_t SET_RADIAL = 0;
uint16_t TRUE_RADIAL = 75;
uint16_t tempval = 0;
...
void read_rotary_encoder(){
  ...
  //static uint8_t rotary_reset; //ensures that a rotation of the encoder does not result in multiple increments/decrements
  static uint8_t rotary_data_lock; //captures the first rising edge from the rotary encoder
  /*SPDR = count_to_therm[(count / 0x007F)]; //convert the count divided by 32 to themometer code and send to HC595
  while(bit_is_clear(SPSR,SPIF)){ //waits for the serial data transfer to finish
	//manually accept the bits into the HC595
	PORTD |= (1<<PORTD2); //set PORTD bit 2 high - rising edge for HC595 storage register
...
void loop() {
  ...
  //erase the needle with a black rectangle
  tft.fillRect(119 + CURRENT_DEFLECTION_POS,60,3,120,TFT_BLACK);

  //draw the indicator if it has updated
  /*if(idk the radial is more than 90 degrees off?){
    indicator.fillSprite(0); // Fill sprite with black
    indicator.fillTriangle(0,0,24,0,12,20,12);
  }*/
  indicator.pushSprite(180,60,0);
  ...
  sradial_text.drawString(String(SET_RADIAL),0,25,4); // Draw string using font 4
  sradial_text.pushSprite(270,0,0);
  tradial_text.pushSprite(0,0,0);

  //use SPI to read what the set radial should be
  read_rotary_encoder();
}
