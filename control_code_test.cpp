...
int set = 0, low_freq = 5000, high_freq = 10800;
unsigned long high_time, low_time, reset;
float time_period=0, angle=0;
float frequency=0, new_freq=0;
...
void loop() {
  ...
  //testing();
    int av = analogRead(2); // ADC12 on GPIO2
      ...
      if (set == true && av >= 2400)
      {high_time = micros()-reset; set = false;}
      else if (set == false && av <= 2400)
      {low_time = micros()-reset; set = true; reset=micros();}

    time_period = low_time-high_time;
   
    new_freq = 15051740400/(time_period);
    if (new_freq >= (low_freq*.9) && new_freq <= (high_freq*1.1))
    frequency = new_freq;

    angle = (360 * (frequency - low_freq)) / (high_freq - low_freq);
    if (angle <= 0) angle=0;
